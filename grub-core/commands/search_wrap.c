...
static const struct grub_arg_option options[] =
  {
    ...
    {"fs-uuid",		'u', 0, N_("Search devices by a filesystem UUID."),
     0, 0},
    {"part-name",	'p', 0, N_("Search devices by a partition name."),
     0, 0},
...
grub_cmd_search (grub_extcmd_context_t ctxt, int argc, char **args)
{
  ...
  if (state[SEARCH_LABEL].set)
    ...
    grub_search_fs_file (id, var, state[SEARCH_NO_FLOPPY].set, 
			 hints, nhints);
  else if (state[SEARCH_PARTNAME].set)
    grub_search_partname (id, var, state[SEARCH_NO_FLOPPY].set,
			 hints, nhints);
  else
  ...
  cmd =
    grub_register_extcmd ("search", grub_cmd_search,
			  ...
			  N_("Search devices by file, filesystem label,"
			     " filesystem UUID or partition name."
