...
int
grub_uboot_tool_gunzip (void *dst, unsigned int dstlen,
			unsigned char *src, unsigned int *lenp)
{
  int retval;

  if (!grub_uboot_syscall (API_TOOL_GUNZIP, &retval, dst, dstlen, src, lenp))
    return 0;
